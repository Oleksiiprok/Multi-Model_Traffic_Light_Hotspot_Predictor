...
# 1. Визначення віджетів виводу для мап (із рамками та стилями)
out_map = widgets.Output(layout={'width': '100%', 'height': '300px', 'border': '2px solid #2980b9'})
...
def render_stable_map(out_widget, map_obj):
    with out_widget:
        ...
        display(HTML(f'<div style="width:100%; height:280px;">{map_obj._repr_html_()}</div>'))
...
def update_dashboard(region, algorithm, period, intersection_idx, hour, day_of_week, temperature, precipitation, traffic_flow, is_roadwork):
    ...
    report_html = f"""
    <div style="background-color: #f8f9fa; padding: 10px; border-left: 4px solid #007bff; border-radius: 4px; margin-bottom: 8px; font-family: sans-serif;">
        <h5 style="margin: 0 0 6px 0; color: #333;">📊 AI Traffic Analysis Report</h5>
        <ul style="margin: 0; padding-left: 18px; color: #444; font-size: 13px;">
            <li><b>Suburb:</b> {region} | <b>Intersection:</b> {selected_loc['name']}</li>
            <li><b>Method:</b> <code>{algo_name}</code> | <b>Period:</b> {period_labels[period]} ({len(filtered_df)} records)</li>
            <li><b>Hotspot Probability:</b> <span style="color: {'red' if probability > 60 else ('orange' if probability > 30 else 'green')}; font-weight: bold;">{probability:.1f}%</span></li>
        </ul>
    </div>
    """
    with out_report:
    ...
    # Create map using user's custom function
    ...
    for i, loc in enumerate(locs_list):
        ...
        tooltip_text = f"Street: {loc['name']} | Risk: {p:.1f}% ({algo_name})"

        folium.CircleMarker(
            location=[loc['lat'], loc['lon']],
            radius=8,
            popup=f"<b>{loc['name']}</b><br>Method: {algo_name}<br>Risk: {p:.1f}%",
...
# 6. Compact Layout Design (Left Sidebar for controls, Right side for Report & Map)
